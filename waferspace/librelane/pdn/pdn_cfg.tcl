...
#  Zotnetic: the template sources one of its two SRAM PDN files here. This chip
#  has no SRAM -- its core is a single analog macro -- and those files define
#  grids for i_chip_core.sram_0 and sram_1, instances that no longer exist, so
#  sourcing them is an error rather than a no-op.
#
#  What goes here instead is the block's own supply. pdn_nav.tcl is GENERATED
#  from the block's LEF by openroad/scripts/waferspace_collateral.py, so if the
#  block is ever re-hardened and its stubs move, the straps move with them.
source [file join [file dirname [info script]] "pdn_nav.tcl"]
