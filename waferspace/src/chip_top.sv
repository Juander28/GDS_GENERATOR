...
module chip_top #(
    ...
    // Schmitt trigger
    //
    // Zotnetic: (* keep *) added. Every other pad in this file has it, through
    // the generate blocks; these two were instantiated bare because the
    // template's core uses clk and rst_n, so something referenced them. This
    // core is one analog macro and references neither, and yosys duly deleted
    // both cells -- which only surfaced three stages later, as
    // "add_global_connections failed to make any connections for
    // 'clk_pad/DVDD'". Deleting them is not an option: they are two of the 11
    // positions in the south row, and the row's count is what fixes where every
    // pad on that edge lands.
    (* keep *)
